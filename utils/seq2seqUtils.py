...
class Example(NamedTuple):
  src: List[str]
  tgt: Optional[List[str]]
  src_len: int  # inclusive of EOS, so that it corresponds to tensor shape
  tgt_len: Optional[int]  # inclusive of EOS, so that it corresponds to tensor shape
...
class TestDataset(object):
  ...
  def __init__(self, srcs:List[str], vocab : Vocab = None,max_src_len: int=None,truncate_src: bool=False):
    ...
    import jieba
    for i, src in tqdm(enumerate(self.srcs),desc = "读入数据集"):
      src = list(
        jieba.cut(src, cut_all=False, HMM=True))
      ...
      self.examples.append(Example(src, None, src_len, None))
    print("%d examples." % len(self.examples))

  def generator(self):
    base_oov_idx = len(self.vocab)
    for i, example in enumerate(self.examples):
      oov_dict = OOVDict(base_oov_idx)
      src_tensor = torch.zeros(example.src_len, 1, dtype=torch.long)
      for j, word in enumerate(example.src):
          idx = self.vocab[word]
          if  idx == self.vocab.UNK:
            idx = oov_dict.add_word(i, word)
          src_tensor[j, 0] = idx
          src_tensor[example.src_len - 1, 0] = self.vocab.EOS
      yield Batch([example], src_tensor, None,[example.src_len], oov_dict)
...
class Dataset:
  def __init__(self, filename: str, tokenize: Callable=simple_tokenizer, max_src_len: int=None,
    ...
    with open(filename, 'rt', encoding='utf-8') as f:
      for i, line in tqdm(enumerate(f),desc = "读入数据集"):
        pair = json.loads(line.strip())
        src = list(
          jieba.cut(pair["src"].replace("<Paragraph>", "。").replace("</Paragraph>", "。"), cut_all=False, HMM=True))
        if max_src_len and len(src) > max_src_len:
          if truncate_src:
            src = src[:max_src_len]
          else:
            continue
        tgt = list(
          jieba.cut(pair["tgt"].replace("<Paragraph>", "。").replace("</Paragraph>", "。"), cut_all=False, HMM=True))
...
<EVAL ID="{1}">
  ...
  <MODELS>
    ...
    with open(config_path, 'wt') as f:
      f.write(xml)
    # run ROUGE
    out = subprocess.check_output('perl ROUGE-1.5.5.pl -e data -a -n 2 -2 4 -u ' + config_path,
